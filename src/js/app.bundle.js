...
(function (global) {
  ...
  // ---------------- 初始化（自动探测后端） ----------------
  // 静默探测：GitHub Pages 等静态部署下 /api/health 会返回 404，
  // 通过 AbortController 在 1.5 秒内超时或拒绝时直接进入 IndexedDB 模式，避免控制台噪音。
  Storage.init = function () {
    detectCaps();
    if (Storage.supports.secure && typeof fetch === 'function') {
      let timedOut = false;
      const ctl = (typeof AbortController !== 'undefined') ? new AbortController() : null;
      const timer = setTimeout(function () { timedOut = true; if (ctl) ctl.abort(); }, 1500);
      const fetchOpts = ctl ? { cache: 'no-store', signal: ctl.signal } : { cache: 'no-store' };
      return fetch('/api/health', fetchOpts)
        .then((r) => r.json())
        .then((j) => {
          clearTimeout(timer);
...
(function (global) {
  ...
  function renderList(mount) {
    ...
    const search = Util.el('input', { type: 'text', id: 'qdLibSearch', name: 'q', class: 'qd-lib-search', placeholder: T('搜索问卷标题…'), value: st.search || '' });
...
(function (global) {
  ...
  UI.render = function (mount) {
    ...
    const searchI = Util.el('input', { type: 'text', id: 'tplSearch', name: 'q', class: 'inp', placeholder: T('搜索模板名称 / 描述…'), style: 'flex:1;min-width:180px' });
...
(function (global) {
  ...
  function buildMapToolbar() {
    ...
    const search = Util.el('input', { id: 'mapNameSearch', name: 'name', class: 'input', placeholder: T('搜索供应商名称/编码'), value: ui.map.name || '' });
...
(function (global) {
  ...
  Standards.render = function (mount) {
    ...
    // ---------- 搜索 ----------
    const search = Util.el('input', { type: 'text', id: 'stdSearch', name: 'q', class: 'std-search', placeholder: T('🔍 搜索标准名称 / 版本 / 标签 / 条款关键词（如 强迫劳动、消防验收、SA8000）') });
    ...
    function renderGlossary() {
      ...
      const gsearch = Util.el('input', { type: 'text', id: 'glossarySearch', name: 'q', class: 'std-search', placeholder: T('🔍 搜索术语（中文 / 英文 / 拼音 / 缩写，如 qpld、smeta、强迫劳动）'), value: gKw });
...
(function (global) {
  ...
  function renderSearchBar(g) {
    ...
    var inp = Util.el('input', {
      id: 'guideSearch', name: 'q', class: 'inp', type: 'text', placeholder: T('搜索本指南内容（标准要求 / 审核要点 / 不符合项 / 整改建议）…'),
      value: gKw
    });
...
(function (g) {
  ...
  PWA.notify = function (title, body, opts) {
    ...
    const payload = {
      ...
      badge: 'src/icons/icon-192x192.png',
...
(function (global) {
  ...
  Auth.renderAuth = function (mode) {
    ...
    const loginU = Util.el('input', { type: 'text', id: 'loginUsername', name: 'username', class: 'inp', placeholder: I18n.t('common.username'), autocomplete: 'username' });
    const loginP = Util.el('input', { type: 'password', id: 'loginPassword', name: 'password', class: 'inp', placeholder: I18n.t('common.password'), autocomplete: 'current-password' });
    const regU = Util.el('input', { type: 'text', id: 'regUsername', name: 'username', class: 'inp', placeholder: I18n.t('login.username_ph') });
    const regN = Util.el('input', { type: 'text', id: 'regDisplayName', name: 'displayName', class: 'inp', placeholder: I18n.t('login.displayName_ph') });
    const regP = Util.el('input', { type: 'password', id: 'regPassword', name: 'password', class: 'inp', placeholder: I18n.t('login.password_ph') });
    const regP2 = Util.el('input', { type: 'password', id: 'regPasswordConfirm', name: 'passwordConfirm', class: 'inp', placeholder: I18n.t('login.passwordConfirm_ph') });    const msg = Util.el('div', { class: 'auth-msg' });
    ...
    const card = Util.el('div', { class: 'auth-card' }, [
      ...
      Util.el('div', { class: 'auth-brand' }, [
        Util.el('img', { class: 'auth-logo', src: 'src/icons/icon-512x512.png', alt: 'SPP' }),
        Util.el('div', {}, [
