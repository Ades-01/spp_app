/* ============================================================================
 * Service Worker —— 供应商预审平台 PWA 核心（SPP 单仓发布版）
 * ----------------------------------------------------------------------------
 ...
 * 所有资源路径使用相对路径，方便 GitHub Pages 在任意子路径（如 /SPP/）下部署。
 * ==========================================================================*/
'use strict';
 ...
 * 相对路径：以 sw.js 所在目录（src/pwa/）为基准；GitHub Pages 子路径下同样生效。 */
const OFFLINE_LIBS = [
  '../src/lib/xlsx.full.min.js', '../src/lib/jspdf.umd.min.js', '../src/lib/jspdf.plugin.autotable.min.js',
  '../src/lib/html2canvas.min.js', '../src/lib/jszip.min.js'
];
const APP_SHELL = [
  '../index.html',
  '../manifest.json',
 ...
 * 部分资源若上线后不可用（例如 404）会阻塞整体 install；
 * 因此采用「先 addAll，单条失败不阻断」策略，确保核心缓存生效。 */
self.addEventListener('install', (event) => {
  event.waitUntil(
    caches.open(CACHE)
      .then((cache) => cache.addAll(APP_SHELL).catch((err) => {
        // 单个资源失败不阻塞安装流程：逐条尝试，失败项跳过。
        console.warn('[SW] App Shell 预缓存部分失败，继续安装:', err);
...
function cacheOnly(req) {
  return caches.match(req).then((hit) => hit || caches.match('offline.html'));
}
...
function offlineFirstNavigation(req) {
  return caches.open(CACHE).then((cache) =>
    cache.match(req, { ignoreSearch: true }).then((cached) => {
      const fetching = fetch(req)
        .then((res) => { if (res && res.status === 200) { cache.put(req, res.clone()); lastSyncTs = Date.now(); } return res; })
        .catch(() => cached || caches.match('../index.html').then((hit) => hit || caches.match('offline.html')));
      return cached || fetching;
    })
  );
}
